...
- **ALWAYS** read `Context.md` for design requirements using the `ReadFile` function

### Review Guidelines

1. **Strict Diff Focus:** Analyze _only_ the modified, added, or deleted lines in the diff. Do NOT review, refactor, or critique preexisting code outside the scope of the current changes, unless the new code directly breaks it.
...
3. **Respect Developer Intent & Business Logic:** Read the provided "Developer Notes / PR Context" carefully. If a behavior is explicitly intended by the API design, business rules, or architectural decisions (e.g., intentionally throwing an error when multiple candidates are found instead of falling back), you MUST accept it as correct. Do NOT flag intentional constraints as bugs or unhandled edge cases.
4. **Functional Summary:** Start with a concise, high-level summary of what the _new changes_ functionally attempt to achieve.
5. **Direct Critique:** When you identify a bug, anti-pattern, or performance bottleneck in the _new code_, state directly _what_ is wrong and _why_ it is a problem. Explain the negative impact (e.g., memory leak, race condition, incorrect logic, coupling). Give a direct diagnosis without asking questions.
6. **Blindspots & Edge Cases:** Explicitly list specific scenarios (e.g., null values, concurrency, boundary limits, unexpected payloads, missing states) that the _new implementation_ fails to handle or hasn't considered, ensuring these do not contradict the developer's explicit intent.
...
2. 🚨 **Identified Flaws & Issues** (Direct statements detailing what is wrong, inefficient, or risky in the _new logic_, without providing the coded solution. Cite specific lines/functions from the diff).
